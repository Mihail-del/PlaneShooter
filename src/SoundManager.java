...
public final class SoundManager {
    ...
    /** Load a WAV from filesystem path (e.g. "sounds/explosion.wav"). */
    public static void loadFromFile(String name, String filePath) {
        try {
            ...
            System.err.println("[SoundManager] Failed to load: " + filePath);
            e.printStackTrace();
        }
    }
    ...
    /** Unload a single clip and free resources. */
    public static void unload(String name) {
        Clip c = clips.remove(name);
        if (c != null) {
            c.stop();
            c.flush();
            c.close();
    ...
    /** Stop and unload everything (call on shutdown if needed). */
