...
public class PlaneShooter extends GraphicsProgram {
    ...
    private void gameOver() {
        ...
        boolean planeCollides = (planeLocationY >= getHeight()*0.8 - PLANE_HEIGHT*0.2) ||
                (planeLocationY >= getHeight()*0.9 - BAR_WIDTH*793/1155.0 && planeLocationX>=getWidth()*0.75) ||
                (planeLocationY >= getHeight()*0.9 - TREE_WIDTH*84/75.0 && planeLocationX<=TREE_WIDTH-getWidth()*0.05);
        if (bar == null && tree == null)
            planeCollides = (planeLocationY >= getHeight()*0.8 - PLANE_HEIGHT*0.2);
        if (bar == null && tree != null)
            planeCollides = (planeLocationY >= getHeight()*0.8 - PLANE_HEIGHT*0.2) ||
                    (planeLocationY >= getHeight()*0.9 - TREE_WIDTH*84/75.0 && planeLocationX<=TREE_WIDTH-getWidth()*0.05);
        if (bar != null && tree == null)
